...
public class ArmorHudOverlay {
    ...
    public void renderArmorUI(DrawContext context) {
        ...
        if (config.isSplitMode()) {
            if (config.isVertical()) {
                // Split vertical: Links (Helmet, Chestplate), Rechts (Leggings, Boots)
                int xOffsetLeft = screenWidth / 2 + config.getXOffsetLeft();
                int yOffsetLeft = screenHeight + config.getYOffsetLeft();
                for (int i = 0; i < 2; i++) {
                    if (!armorItems[i].isEmpty()) {
                        drawBoxAndArmor(context, armorItems[i], xOffsetLeft, yOffsetLeft, boxSize, spacing, i, true);
                    }
                }
                ...
                for (int i = 0; i < 2; i++) {
                    if (!armorItems[i].isEmpty()) {
                        drawBoxAndArmor(context, armorItems[i], xOffsetLeft, yOffsetLeft, boxSize, spacing, i, false);
                    }
                }
                int xOffsetRight = screenWidth / 2 + config.getXOffsetRight();
                int yOffsetRight = screenHeight + config.getYOffsetRight();
            ...
            for (int i = 0; i < 4; i++) {
                if (!armorItems[i].isEmpty()) {
                    drawBoxAndArmor(context, armorItems[i], xOffset, yOffset, boxSize, spacing, i, config.isVertical());
                }
            }
        }
    }
    ...
    public static void renderPreview(DrawContext context, int screenWidth, int screenHeight, ArmorHudConfig config) {
        ...
        if (config.isSplitMode()) {
            if (config.isVertical()) {
                ...
                for (int i = 0; i < 2; i++) {
                    if (!previewArmor[i].isEmpty()) {
                        (new ArmorHudOverlay()).drawBoxAndArmor(context, previewArmor[i], xOffsetLeft, yOffsetLeft, boxSize, spacing, i, true);
                    }
                }
                int xOffsetRight = screenWidth / 2 + config.getXOffsetRight();
                int yOffsetRight = screenHeight + config.getYOffsetRight();
                ...
                for (int i = 0; i < 2; i++) {
                    if (!previewArmor[i].isEmpty()) {
                        (new ArmorHudOverlay()).drawBoxAndArmor(context, previewArmor[i], xOffsetLeft, yOffsetLeft, boxSize, spacing, i, false);
                    }
                }
                int xOffsetRight = screenWidth / 2 + config.getXOffsetRight();
                int yOffsetRight = screenHeight + config.getYOffsetRight();
            ...
            for (int i = 0; i < 4; i++) {
                if (!previewArmor[i].isEmpty()) {
                    (new ArmorHudOverlay()).drawBoxAndArmor(context, previewArmor[i], xOffset, yOffset, boxSize, spacing, i, config.isVertical());
                }
            }
        }
    }
