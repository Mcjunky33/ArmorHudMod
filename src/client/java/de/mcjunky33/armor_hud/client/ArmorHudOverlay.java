...
public class ArmorHudOverlay {
    ...
    public void renderArmorUI(DrawContext context) {
        ...
        if (config.isVertical()) {
            if (config.isSplitMode()) {
                // LEFT (item 0–1)
                int xOffsetLeft2 = config.getXOffsetLeft2();
                int yOffsetLeft2 = config.getYOffsetLeft2();
                for (int i = 0; i < 2; i++) {
                    if (!armorItems[i].isEmpty()) {
                        drawBoxAndArmor(context, armorItems[i],
                                xOffsetLeft2, yOffsetLeft2,
                                boxSize, spacing, i, true,
                                config.getNumberOffsetXLeft(), config.getNumberOffsetYLeft());
                    }
                }

                // RIGHT (item 2–3)
                int xOffsetRight = config.getXOffsetRight();
                int yOffsetRight = config.getYOffsetRight();
                for (int i = 2; i < 4; i++) {
                    if (!armorItems[i].isEmpty()) {
                        drawBoxAndArmor(context, armorItems[i],
                                xOffsetRight, yOffsetRight,
                                boxSize, spacing, i - 2, true,
                                config.getNumberOffsetXRight(), config.getNumberOffsetYRight());
                    }
                }

            } else {
                // VERTICAL + NORMAL (items 0–3), komplett verschiebbar
                int xOffset = config.getXOffset(); // jetzt richtig gesetzt
                int yOffset = config.getYOffset();

                for (int i = 0; i < 4; i++) {
                    if (!armorItems[i].isEmpty()) {
                        int itemY = yOffset + i * (boxSize + spacing);
                        drawBoxAndArmor(context, armorItems[i],
                                xOffset, itemY,
                                boxSize, spacing, i, true,
                                config.getNumberOffsetX(), config.getNumberOffsetY());
                    }
                }
                }
    ...
    private void drawBoxAndArmor(DrawContext context, ItemStack armorItem, int baseX, int baseY, int boxSize, int spacing, int idx, boolean vertical, int offsetXNumber, int offsetYNumber) {
        ...
        // Durability-Anzeige: Modus 0=Bar, 1=Zahl, 2=Prozent
    ...
    /**
     * Zeigt Durability als Zahl oder Prozent an.
     * offsetXNumber: Verschiebung der Zahl nach links/rechts.
     * offsetYNumber: Verschiebung der Zahl nach oben/unten.
     * showAsPercent: true = Prozentanzeige, false = Zahl
     */
    ...
    public static void renderPreview(DrawContext context, int screenWidth, int screenHeight, ArmorHudConfig config) {
        ...
        // Setze die Schadenswerte für Vorschau
            ...
            if (config.isSplitMode()) {
                ...
                for (int i = 0; i < 2; i++) {
                    if (!previewArmor[i].isEmpty()) {
                        int itemY = yOffsetLeft2 + i * spacing; // spacing nur
                        (new ArmorHudOverlay()).drawBoxAndArmor(context, previewArmor[i], xOffsetLeft2, itemY, boxSize, spacing, i, true, config.getNumberOffsetXLeft(), config.getNumberOffsetYLeft());
                    }
                }
                ...
                for (int i = 2; i < 4; i++) {
                    if (!previewArmor[i].isEmpty()) {
                        int itemY = yOffsetRight + (i - 2) * spacing; // spacing nur
                        (new ArmorHudOverlay()).drawBoxAndArmor(context, previewArmor[i], xOffsetRight, itemY, boxSize, spacing, i - 2, true, config.getNumberOffsetXRight(), config.getNumberOffsetYRight());
                    }
                }
            } else {
                // VERTICAL + NORMAL (items 0–3)
                int xOffset = screenWidth / 2 + config.getXOffsetLeftVertical();
                int yOffset = screenHeight + config.getYOffsetLeftVertical();
                for (int i = 0; i < 4; i++) {
                    if (!previewArmor[i].isEmpty()) {
                        int itemY = yOffset + i * spacing; // spacing nur
                        (new ArmorHudOverlay()).drawBoxAndArmor(context, previewArmor[i], xOffset, itemY, boxSize, spacing, i, true, config.getNumberOffsetXLeftVertical(), config.getNumberOffsetYLeftVertical());
                    }
                }
            }
        } else if (config.isSplitMode()) {
